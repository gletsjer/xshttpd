...
typedef ssize_t xs_write_fun(const char *, const size_t);
...
bool	ssi_init(void);
bool	ssi_handler(char *, int, xs_error_t *, off_t *);
bool	ssi_config_general(const char *, const char *);

#define		MAXINCLUDES	16
#define		CONDKEYWORDS	16
#define		SETVARIABLES	200
#define		SSIARGUMENTS	100
static	bool	do_ssi = true;
...
xsc_initdummy(off_t *size)
{
	...
	{
		*size += printf("[Failed to create dummies: %s]\n",
			strerror(errno));
		return false;
	}
	...
	{
		*size += printf("[Failed to write dummy file: %s]\n",
			strerror(errno));
		close(fd);
		return false;
	}
...
xsc_initcounter(const char *filename, off_t *size)
{
	...
	if ((fd = open(datafile, O_RDONLY,
		S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH)) < 0)
	{
		*size += printf("[Could not open the counter file: %s]\n",
			strerror(errno));
		return false;
	}
	retry = 0;
	while ((fd2 = open(lockfile = CNT_LOCK,
	...
	if (fd2 < 0)
	{
		*size += printf("[Failed to create temporary file: %s]\n",
			strerror(errno));
		close(fd);
		return false;
	}
	...
	while (read(fd, &counter, sizeof(counter)) == sizeof(counter))
	{
		if ((!done) && (strcmp(counter.filename, filename) > 0))
		{
			if (write(fd2, &counter2, sizeof(counter2)) !=
				sizeof(counter2))
			{
				*size += printf("[Failed to write temp file: %s]\n",
					strerror(errno));
	...
	{
		*size += printf("[Could not rename counter file: %s]\n",
			strerror(errno));
		remove(lockfile);
		return false;
	}
...
xsc_counter(countermode mode, const char *args, off_t *size)
{
	...
	{
		*size += printf("[Could not find end of the counter file: %s]\n",
			strerror(errno));
		close(fd);
		return false;
	}
	...
	while ((x < (z-1)) && (comp))
	{
		...
		{
			*size += printf("[Could not seek in counter file: %s]\n",
				strerror(errno));
			close(fd);
			return false;
		}
		...
		{
			*size += printf("[Could not read counter file: %s]\n",
				strerror(errno));
			close(fd);
			return false;
		}
	...
	if (comp)
	{
		...
		if (already)
		{
			*size += printf("[Failed to create new counter]\n");
			return false;
		}
		already = true;
	...
	if (mode == MODE_RESET)
	{
		...
		{
			*size += printf("[Could not update counter file: %s]\n",
				strerror(errno));
			close(fd);
			return false;
		}
	...
	{
	case MODE_ALL:
		*size += printf("%d", counter.total);
		break;
	case MODE_GFX_ALL:
		*size += printf("<IMG SRC=\"/%s/gfxcount%s?%d\" ALT=\"%d\">",
			current->execdir,
			args ? args : "", counter.total, counter.total);
		break;
	case MODE_TODAY:
		*size += printf("%d", counter.today);
		break;
	case MODE_GFX_TODAY:
		*size += printf("<IMG SRC=\"/%s/gfxcount%s?%d\" ALT=\"%d\">",
			current->execdir,
	...
	case MODE_RESET:
		if (counter.total > 0)
			...
			goto REOPEN;
		*size += printf("[reset stats counter]");
		break;
	}
	return true;
}
...
dir_date_format(int argc, char **argv, off_t *size)
{
	...
	if (!argc)
	{
		*size += puts("[No parameter to date-format]\n");
		return(ERR_CONT);
	}
...
dir_date(int argc, char **argv, off_t *size)
{
	...
	*size += strftime(buffer, MYBUFSIZ - 1, format, localtimenow());
	if (ozone)
	{
		...
		unsetenv("TZ");
	return(puts(buffer) == EOF ? ERR_QUIT : ERR_NONE);
}
...
dir_include_file(int argc, char **argv, off_t *size)
{
	...
	{
		*size += puts("[Too many include files]\n");
		return(ERR_CONT);
	}
	if (!argc)
	{
		*size += puts("[No parameter for include-file]\n");
		return(ERR_CONT);
	}
	...
	if (fd < 0)
	{
		...
		*size += printf("[Error opening file `%s': %s]\n",
	...
	else /* dump content directly */
	{
		...
		{
			write(STDOUT_FILENO, buffer, (size_t)rlen);
			*size += rlen;
		}
		ret = 0;
	}
...
dir_last_mod(int argc, char **argv, off_t *size)
{
	...
	if (argc)
	{
		...
		{
			escpath = escape(path);
			*size += printf("[Cannot stat file '%s': %s]\n",
				escpath, strerror(errno));
			FREE(escpath);
			return(ERR_CONT);
		}
...
dir_exec(int argc, char **argv, off_t *size)
{
	...
	if (!argc)
	{
		*size += puts("[No parameter for exec]\n");
		return(ERR_CONT);
	}

	if (!strcmp(argv[0], "cgi"))
	{
		*size += puts("[exec cgi not supported: use run-cgi]\n");
		return(ERR_CONT);
	}
	...
	{
		*size += puts("[exec invalid argument: use exec cmd=..]\n");
		return(ERR_CONT);
	}
	...
	case 0:
		 ...
		 */
		*size += puts("[exec not implemented]\n");
		exit(1);
	case -1:
		*size += printf("[Execute failed: %s\n", strerror(errno));
		return(ERR_CONT);
	default:
...
dir_run_cgi(int argc, char **argv, off_t *size)
{
	...
	if (!argc)
	{
		*size += puts("[No parameter for run-cgi]\n");
		return(ERR_CONT);
	}
...
dir_set(int argc, char **argv, off_t *size)
{
	...
	{
		*size += puts("[Too many set arguments]\n");
		return(ERR_CONT);
	}
...
dir_if(int argc, char **argv, off_t *size)
{
	...
	{
		*size += puts("[No parameters for if]\n");
		return(ERR_CONT);
	}
	if (ssioutput >= SSIARGUMENTS-1)
	{
		*size += puts("[Too many nested if statements]\n");
		return(ERR_CONT);
	}
...
dir_switch(int argc, char **argv, off_t *size)
{
	if (!argc)
	{
		*size += puts("[No parameter for switch]\n");
		return(ERR_CONT);
	}
...
parsedirectives(char *parse, off_t *size)
{
	...
	while (*here)
	{
		...
		if (store != result)
		{
			if (printable)
			{
				if (write(STDOUT_FILENO, result, (size_t)(store - result)) < 0)
					return(ERR_QUIT);
				*size += (store - result);
			}
			store = result;
		...
		for (directive = directives; directive->name; directive++)
		{
			...
			if (printable || directive->conditional)
			{
				...
				case ERR_QUIT:
					return(ERR_QUIT);
				case ERR_CONT:
					*size += puts("[Error parsing directive]\n");
					break;
				}
			}
...
sendwithdirectives_internal(int fd, off_t *size)
{
	...
	while ((line = fgetln(parse, &sz)))
	{
		if (!memmem(line, sz, "<!--#", 5))
		{
			if (print_enabled())
			{
				if (write(STDOUT_FILENO, line, sz) == EOF)
				{
					alarm(0);
					fclose(parse);
					return(ERR_QUIT);
...
bool
ssi_handler(char *filename, int fdin, xs_error_t *xserr, off_t *size)
{
	*size = 0;
	...
	*xserr = sendwithdirectives_internal(fdin, size);
...
bool
ssi_config_general(const char *key, const char *value)
{
	if (key && !strcasecmp("UseServerSideInclude", key))
	{
		do_ssi = !strcasecmp("true", value);
		return true;
	}
	return false;
}

struct module ssi_module =
{
	.name = "ssi interpreter",
	.engine = "internal:ssi",
	.init = ssi_init,
	.html_handler = ssi_handler,
	.config_general = ssi_config_general,
};
