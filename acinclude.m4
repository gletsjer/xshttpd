...
AC_DEFUN([XS_ARG_DIRS], [
	m4_foreach_w([loc], [$1], [
		CPPFLAGS="${CPPFLAGS} -[D]AS_TR_CPP(loc[_DIR])=\\\"${loc[dir]}\\\""
...
dnl AC_FUNC_IN_LIB(function, define, library, buildprog, extra-lib,
...
AC_DEFUN([XS_CHECK_PC], [
	PKG_CHECK_MODULES(AS_TR_SH($1), [lib$1], [
		...
		$2_ldadd="${$2_ldadd} ${AS_TR_SH($1_LIBS)}"
...
AC_DEFUN([XS_TRY_CONFIG], [
		...
		$2_ldadd="${$2_ldadd} `${AS_TR_SH(xs_$1_path)} --libs`"
		AC_DEFINE_UNQUOTED(AS_TR_CPP(HAVE_$1), 1,
			[Define this if you have the $1 libary])
		$3
		], [
